...
1. **The experimenter is an untrusted party.** A credible result cannot require anyone to trust the server operator (the experimenter, the host, or anyone). An independent auditor must be able to verify the results from published artifacts alone. This single principle drives most of the cryptographic design below.
...
From the original project brief; recorded verbatim-in-spirit, to be refined:
...
Each item: the question, options, a recommendation, and status. **OPEN** items need a project decision before they affect build work.
...
No budget exists day one. Path: (1) **today, $0** — pilot on `RDSEED` (real physical, in the host CPU); (2) **~$35–50** — add an open-hardware USB TRNG (*Infinite Noise* / *OneRNG*) for auditable, citable early data; (3) **later** — a quantum RNG for the flagship confirmatory dataset. Source-agnostic design now; device choice locked before Phase 3 confirmatory collection.

### D12 — Experimenter-as-subject safeguards `DECIDED (separation adopted)`
An experimenter who is also a putative subject is a known credibility risk (cf. PEAR "Operator 10"). PsiMeter adopts the strongest safeguard: **the experimenter role is excluded from the confirmatory subject pool entirely** — confirmatory data come only from independent public participants, and any operator key used by the experimenter role is pre-declared and excluded a priori. More generally:
- Every operator (including anyone who plays only the *exploratory* firehose) uses an ordinary **pseudonymous operator key**; all sessions are pre-committed/immutable alike.
- The confirmatory analysis is a **frozen, hash-anchored, deterministic script over the public ledger**, so its result does not depend on who runs it; an **independent party is invited to co-sign** the freeze and the ledger anchors.
- *Framed positively:* the same machinery that defends against a fraudulent experimenter also guards against self-deception — which is the stated goal (prove **or** disprove).
