...
override with `PSIMETER_ENTROPY=os|rdseed`. Still stubbed: operator Ed25519
signing, a live public beacon, and raw-blob persistence + external anchoring.

```bash
npm run build:core    # the server imports the built @psimeter/core
npm start             # http://localhost:8787 — run a session in the browser

# headless transport check (skips human-paced delays):
PSIMETER_FAST=1 PSIMETER_ENTROPY=os npm run smoke
```
