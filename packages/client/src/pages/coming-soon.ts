...
// what's coming, and what they can do right now (read the docs, read the code).
  ...
  outlet.append(
    el('div', { class: 'page prose narrow' }, [
      el('section', { class: 'soon-hero' }, [
        ...
        el('p', { class: 'lede' },
          'The PsiMeter platform is still being built. Running live experiments, the leaderboard, your profile, and in-browser verification all need the server, which isn’t open to the public yet. The science and the cryptographic core are finished — the public site is the part in progress.'),
...
function card(title: string, body: string, href: string): HTMLElement {
  ...
  return el('a', internal
    ? { class: 'pillar card soon-card', href, 'data-link': true }
    : { class: 'pillar card soon-card', href },
    [
      el('h3', {}, title),
      el('p', {}, body),
    ]);
