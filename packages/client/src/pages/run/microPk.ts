// The gamified micro-PK experiment runner (spec §10).
//
...
//
// One of several experiment kinds (see pages/run.ts, which dispatches by kind).
...
export function renderMicroPkRunner(outlet: HTMLElement, info: ExperimentInfo): Disposer {
  ...
  function goLive(session: CreatedSession): void {
    ...
    const params = session.params as unknown as MicroPkParams;
    const meta = INTENTIONS[session.intention as Intention];

    const zVal = el('span', { class: 'v tnum' }, '0.00');
    const timeVal = el('span', { class: 'v tnum' }, fmtTime(params.sessionSeconds));
