...
Tests the owner's hypothesis (and a long line of presentiment studies): that some people feel an event's **emotional valence before it happens**. A session is ~2 minutes of **fixed-N forced-choice trials**; each trial the operator picks between two emotionally-loaded outcomes (e.g. *calm* vs *dread*) **before the target exists**, then it is revealed and scored. We hunt for individuals who beat chance **consistently** — this is **H1** (per-operator hit-rate + split-half reliability), not a one-session result.

Per-trial sequence (`packages/server/src/kinds/precog.ts`), two-way WebSocket:
1. server → `trial`; the operator feels and chooses.
...
4. server waits for `R` to publish, **BLS-verifies** `B_R`, derives `target = SHA256(B_R ‖ uint32(trialIndex))[:8] mod k` (`derivePrecogTarget`, mirrored in `analyze.py`), scores the hit, and reveals.

The **target source is the future drand round only** (decision below) — fully reproducible by anyone, no trust in the server's entropy. Trials fold into a per-session Merkle root; the trial list is the content-addressed raw blob. Two-way comms is sound here precisely because the target is bound to a round nobody can predict at choice time, so there is no channel to game (contrast micro-PK's one-way isolation). Every trial is re-verifiable offline and **in-browser** (`/verify` re-derives all targets, checks each signature and that `R > R0`, and reproduces the Merkle root). Residual: the server could backdate when it received a choice — the same §7.4 timing attack, closed only by live witnesses (Phase 3).
...
- **Multi-experiment platform `BUILT`:** the stack was generalized to an **experiment-kind registry** (D14) with micro-PK behaviour and every committed hash unchanged (frozen golden-vector tests guard against drift), and the **presentiment / forced-choice precognition** experiment (§7.5) was added end-to-end — its own server runner, a 2-minute commit→reveal client runner, hit-rate scoring, and full re-verification both in `analyze.py` (per-trial targets, signatures, Merkle) and **in-browser** (`/verify` fetches the small trial blob and re-derives every target from the public beacon). `/run` now routes by `?experiment=<id>&v=<n>`; the experiments/leaderboard/history views are choice-vocabulary-agnostic.
